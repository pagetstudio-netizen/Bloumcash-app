...
interface Promo {
  ...
  buttonActionType: ActionType | null;
...
function PromoCard({ promo, delay }: { promo: Promo; delay: number }) {
  ...
  const isSoon = promo.badge === "soon";
  const hasButton = !isExpired && !isSoon && promo.buttonText && promo.buttonActionType && promo.buttonActionType !== "none" && promo.buttonUrl;

  const handleButton = () => {
    if (!promo.buttonUrl) return;
    if (promo.buttonActionType === "link") {
      window.open(promo.buttonUrl, "_blank", "noopener,noreferrer");
    } else if (promo.buttonActionType === "page") {
      setLocation(promo.buttonUrl);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        {promo.description && (
          <p className="text-[13px] text-gray-500 leading-relaxed mb-0">{promo.description}</p>
        )}
        {hasButton && (
          <motion.button
            whileTap={{ scale: 0.97 }}
          ...
          >
            {promo.buttonText} {promo.buttonActionType === "link" ? "↗" : "→"}
          </motion.button>
        )}
      </div>
    </motion.div>
