...
const SHARE_URL = "https://bloumcash.com/télécharger";
...
const platforms = [
  ...
  },
  {
    name: "Telegram",
    color: "#229ED9",
    url: () => `https://t.me/share/url?url=${encodeURIComponent(SHARE_URL)}&text=${encodeURIComponent("Salut ! Je vous recommande cette super application Bloum Cash, elle permet de transférer de l'argent entre TMoney et Moov. Les paiements sont instantanés !")}`,
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
  ...
  },
  {
    name: "SMS",
    color: "#34C759",
    url: () => `sms:?body=${encodeURIComponent(SHARE_TEXT)}`,
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
  ...
  },
  {
    name: "Email",
    color: "#EA4335",
    url: () => `mailto:?subject=${encodeURIComponent("Essaie Bloum Cash !")}&body=${encodeURIComponent(SHARE_TEXT)}`,
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
